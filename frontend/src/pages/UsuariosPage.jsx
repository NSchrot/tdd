...
export function UsuariosPage() {
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      if (editando) {
        const atualizado = await usuarioService.atualizarUsuario(editando.id, payload);
        ...
        const criado = await usuarioService.criarUsuario(payload);
        setUsuarios(prev => [criado, ...prev.filter(u => u.id !== criado.id)]);
        setPagina(1);
