...
export function LivrosPage() {
  ...
  const salvar = async (e) => {
    ...
    try {
      if (editando) {
        const atualizado = await livroService.atualizarLivro(editando.id, form);
        setLivros(prev => prev.map(l => l.id === editando.id ? atualizado : l));
      } else {
        const criado = await livroService.criarLivro(form);
        setLivros(prev => [criado, ...prev.filter(l => l.id !== criado.id)]);
        setPaginaAtual(1);
      }
