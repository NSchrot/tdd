...
export function MultasPage() {
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      if (editando) {
        const atualizada = await multaService.atualizarMulta(editando.id, payload);
        setMultas(prev => prev.map(m => m.id === editando.id ? atualizada : m));
        setMsg('Multa atualizada.');
      } else {
        const criada = await multaService.criarMulta(payload);
        setMultas(prev => [criada, ...prev.filter(m => m.id !== criada.id)]);
        setPagina(1);
